...
// Google OAuth - frontend sends Google credential (ID token)
router.post('/google', async (req, res) => {
  try {
    ...
    // Decode the JWT from Google (no secret needed for basic decode - Google's public key verifies it)
    ...
    if (req.isDemoMode) {
      let user = req.demoDB.findUserByEmail(email);
      if (!user) {
        const username = name || email.split('@')[0];
        user = req.demoDB.createUser({ username, email, password: 'google-oauth-' + googleId });
        user.avatar = picture || '';
      }
