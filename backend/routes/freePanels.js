...
router.post('/:id/download', auth, async (req, res) => {
  try {
    ...
    await Download.create({ panelId: panel._id, panelName: panel.name, userId: req.user._id, ipAddress: req.ip, userAgent: req.get('User-Agent'), shortenerLink: panel.shortenerLink });
    return res.json({
      success: true,
      requiresShortener: true,
      shortenerLink: panel.shortenerLink,
...
router.post('/:id/confirm-download', auth, async (req, res) => {
  try {
    if (req.isDemoMode) {
      const panel = req.demoDB.findFreePanelById(req.params.id);
      if (!panel) return res.status(404).json({ success: false, message: 'Panel not found' });
      ...
      const download = req.demoDB.recordDownload(panel._id, panel.name, req.user._id);
      return res.json({
        success: true,
    ...
    const download = await Download.create({ panelId: panel._id, panelName: panel.name, userId: req.user._id, ipAddress: req.ip, userAgent: req.get('User-Agent'), shortenerCompleted: true, downloadCompleted: true });
