...
function FloatingOrbs() {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      <div className="absolute -top-40 -right-40 w-96 h-96 bg-primary-400/10 rounded-full blur-3xl animate-float-slow" />
      <div className="absolute -bottom-40 -left-40 w-80 h-80 bg-accent-500/10 rounded-full blur-3xl animate-float-slow" style={{ animationDelay: '1.5s' }} />
      <div className="absolute top-1/3 left-1/4 w-48 h-48 bg-sky-400/8 rounded-full blur-3xl animate-float" style={{ animationDelay: '0.8s' }} />
      <div className="absolute bottom-1/3 right-1/4 w-56 h-56 bg-cyan-400/8 rounded-full blur-3xl animate-float" style={{ animationDelay: '2.2s' }} />
    </div>
  );
}

function FreePanelsContent() {
  ...
  const fetchPanels = async () => {
    ...
    try {
      ...
      if (sortBy === 'popular') {
        const res = await axios.get(`${API_URL}/free-panels/popular`);
        setPanels(res.data.data || []);
        setTotalPages(1);
      } else {
        const res = await axios.get(`${API_URL}/free-panels`, { params });
        setPanels(res.data.data || []);
  ...
  const fetchAllPanels = async () => {
    try {
      const res = await axios.get(`${API_URL}/free-panels`, { params: { limit: 1000 } });
  ...
  return (
    <div className="pt-24 pb-16 min-h-screen">
      ...
      <div ref={headerRef} className="relative py-24 md:py-32 mb-8 overflow-hidden min-h-[350px]" style={{ perspective: '1000px' }}>
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-primary-500/10 via-sky-500/5 to-accent-500/10" />
        <FloatingOrbs />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <motion.div
              ...
              className="inline-flex items-center space-x-2 px-4 py-2 rounded-full bg-primary-100 dark:bg-primary-900/30 text-primary-600 dark:text-primary-300 text-sm font-medium mb-4"
            >
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-4" style={{ transform: 'translateZ(40px)' }}>
              Free <span className="gradient-text">Panels</span>
            </h1>
            <p className="text-lg font-medium text-gray-900 dark:text-white max-w-2xl mx-auto" style={{ transform: 'translateZ(20px)' }}>
              Browse and download our collection of free panels. 
              No strings attached — just quality tools for your needs.
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="text-sm text-gray-500 dark:text-gray-400">
            {loading ? 'Loading...' : `${panels.length} panels found`}
          </span>
        </motion.div>
        ...
        {loading ? (
          ...
          >
            <div className="w-20 h-20 mx-auto rounded-2xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center mb-4">
              <HiDownload className="w-10 h-10 text-gray-400" />
            </div>
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">No panels found</h3>
          ...
          >
            {panels.map((panel, i) => (
              <PanelCard key={panel._id} panel={panel} type="free" index={i} />
            ))}
          </motion.div>
        )}
...
export default function FreePanelsPage() {
  return (
    <Suspense fallback={
